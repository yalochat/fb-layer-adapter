...
module.exports = class Messenger extends AdapterInterface {
  ...
  constructor (options) {
    ...
    this.logger.info(`Layer adapter created with token ${this.config.token}`)
      ...
      .then(templates => {
        ...
        return new Promise((resolve, reject) => {
          ...
          this.wreck.post(url, {payload: JSON.stringify(templateData)}, (err, response, payload) => {
            ...
            this.logger.info(`Request response ${response.statusCode} -> ${payload}`)
  ...
  sendTextFromHook (payload) {
    ...
    this.logger.info('Filtering for user Id ---------------->', payload.message.sender.user_id)

    if (payload.message.sender.user_id == 'service.yalochat' || internals.regex.operators.test(payload.message.sender.user_id)) {
      ...
      return Helpers.errorPromise(`Ignoring message from user or bot ${payload.message.sender.user_id}`)
    }
  }
}
