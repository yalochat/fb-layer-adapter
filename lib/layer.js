...
module.exports = class Layer extends MsgInterface {
    ...
    _validateConversation(recipient) {
        ...
        let participants = _.union([this.config.sender.id], this.config.participants, this.recipient)

        Logger.info(`A conversation will be created with the following participants ${participants}`)
    ...
    _sendMessage (recipient, message, url, parContentType){
        ...
        Logger.info('in _sendMessage content/type', contentType)

        return new Promise((resolve, reject) => {

            const payload = {
                sender:{
                    user_id: this.config.sender.id
                },
                parts:[
                    {
                        body: message,
            ...
            Logger.info('in _sendMessage payload', payload)
