...
module.exports = class Layer extends AdapterInterface {
  ...
  constructor (options) {
    ...
    this._getLoggerInstance({console: true, name: '/lib/layer'})
  ...
  sendTextFromHook (payload, botId) {

    const data = payload.entry[0].messaging[0]
    const message = data.message

    //Handle sender and user for echo messages
    const senderId = _.has(message,'is_echo') && botId ? `${botId}.${data.sender.id}.bot` : data.sender.id
    const userId = _.has(message,'is_echo') ? data.recipient.id : data.sender.id

    this.logger.info('in sendTextFromHook with payload ->', data)

    if (message && message.text) {

      this.logger.info('Executing reverse proxy with text')
      return this.sendText(senderId, message.text, null, userId)
    } else if (data.postback && data.postback.payload) {

      this.logger.info('Executing reverse proxy with postback')
      return this.sendText(senderId, `Usuario selecciono: ${data.postback.payload}`, null, userId)
      ...
      return Promise.map(message.attachments, (attachment) => {
        if (attachment.type == 'image') {
          return this.sendText(senderId, attachment.payload.url, null, userId)
        }
      })
    }
  ...
  sendText (recipient, message, contentType, user) {

    return this._validateConversation(user)
      .then(conversation => {
